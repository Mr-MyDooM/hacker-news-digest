...
var (
	...
	// Performance: pre-lowercased patterns to avoid redundant strings.ToLower calls in isBlockedContent.
...
func prefixSummary(content string, maxLen int) string {
	...
	// This avoids expensive []rune(content) allocation.
	byteIdx := 0
	for i := 0; i < maxLen; i++ {
		_, size := utf8.DecodeRuneInString(content[byteIdx:])
		byteIdx += size
	}
	return content[:byteIdx] + " ..."
}
