...
func TestGetSafeClientRedirect(t *testing.T) {
	...
	// Test restricted protocol
	u, _ := url.Parse("ftp://example.com")
	req2 := &http.Request{URL: u}
	err = client.CheckRedirect(req2, nil)
	if err == nil || !strings.Contains(err.Error(), "restricted redirect protocol") {
		t.Errorf("expected error for ftp redirect, got %v", err)
	}
}
