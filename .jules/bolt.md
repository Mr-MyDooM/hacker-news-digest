...
## 2026-05-20 - Cached Template Rendering
**Learning:** Re-parsing HTML templates and re-allocating the `FuncMap` on every page render adds significant overhead, especially when generating many pages in a single run. Pre-parsing templates once using `sync.Once` and caching them at the package level eliminates redundant I/O and parsing logic.
**Action:** Always pre-parse and cache templates and their associated function maps at the package level for applications that perform repeated rendering.
