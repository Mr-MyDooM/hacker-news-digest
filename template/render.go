...
func RenderFeed(newsList []*hn.News, siteURL string) string {
	...
	for _, news := range newsList {
		...
		summaryLink := fmt.Sprintf(" <a href=\"%s/#%s\">[summary]</a>", escapeXML(siteURL), escapeXML(news.Slug()))
		commentsLink := ""
		if news.CommentURL != "" {
			commentsLink = fmt.Sprintf(" <a href=\"%s\">[comments]</a>", escapeXML(news.CommentURL))
		}

		// news.Summary is plain text from LLM or article extractor.
		// It must be escaped here because it's being included in an HTML content block.
		...
		b.WriteString(fmt.Sprintf("    <id>%s/#%s</id>\n", escapeXML(siteURL), escapeXML(news.Slug())))
