...
var (
	...
	// Performance: xmlReplacer is pre-allocated to avoid multiple strings.ReplaceAll calls and redundant allocations.
...
var globalFuncMap = template.FuncMap{
	...
	"truncateSummary": func(s string, m db.Model) string {
		...
		// Performance: Using a range loop to find the truncation point avoids expensive []rune(s) conversions
		// and redundant heap allocations.
		limit := 400
		count := 0
		for i := range s {
			if count == limit {
