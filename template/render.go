...
var (
	...
	// Performance: xmlReplacer is used for single-pass XML escaping to avoid multiple string allocations.
...
var globalFuncMap = template.FuncMap{
	...
	"truncateSummary": func(s string, m db.Model) string {
		if m.CanTruncate() {
			// Performance: Use a range loop to count runes instead of []rune(s) conversion.
			// This avoids O(N) memory allocation and copying of the entire string.
			count := 0
			for i := range s {
				count++
				if count > 400 {
